...
const PARAMETER_EXAMPLES = {
  ...
  emails: [ 'info@github.com' ]
}

function addCodeSamples ({ routes, baseUrl }) {
  routes.forEach(route => {
    const scope = getScope(route.operation.externalDocs.url)
    const idName = getIdName(route, scope)
    const codeSampleParams = { route, scope, idName, baseUrl }
...
function toShellExample ({ route, scope, idName, baseUrl }) {
  const pathParams = mapValues(
    getExamplePathParams(route),
    (value, paramName) => PARAMETER_EXAMPLES[paramName]
      ? value
...
function toJsExample ({ route, scope, idName, baseUrl }) {
  ...
  return `octokit.${method}(${Object.keys(params).length ? stringify(params, null, 2) : ''})`
}
...
function getExampleBodyParams ({ operation }) {
  ...
  const props = schema.required && schema.required.length > 0
    ? schema.required
...
function getExampleParamValue (name, schema) {
  ...
  switch (schema.type) {
    ...
    case 'object':
      return mapValues(
        schema.properties,
        (propSchema, propName) => getExampleParamValue(propName, propSchema)
      )
    case 'array':
