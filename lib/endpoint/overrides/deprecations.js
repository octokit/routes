...
function deprecations (state) {
  ...
  if (searchIssuesAndPullRequests) {
    ...
    deprecated.deprecated = {
      ...
      after: {
        idName: 'search-issues-and-pull-requests'
      }
    }
    state.results.push(deprecated)
  }

  // 2019-01-05 – "and" is no longer ignored for "idName"
  const getOrCreateAuthorizationForAppAndFingerprint = findByRoute(state, 'PUT /authorizations/clients/:client_id/:fingerprint')
  if (getOrCreateAuthorizationForAppAndFingerprint) {
    ...
    deprecated.idName = 'get-or-create-authorization-for-app-and-fingerprint'
    deprecated.deprecated = {
      date: '2018-12-27',
  ...
  const provisionAndInviteUsers = findByRoute(state, 'POST /scim/v2/organizations/:org/Users')
  if (provisionAndInviteUsers) {
    const deprecated = Object.assign({}, provisionAndInviteUsers)
    deprecated.idName = 'provision-and-invite-users'
    deprecated.deprecated = {
      date: '2018-12-27',
...
function findByRoute (state, route) {
  return state.results.find(endpoint => {
