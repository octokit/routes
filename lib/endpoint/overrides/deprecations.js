...
function deprecations({ routes, gheVersion }) {
  ...
  if (!gheVersion) {
    const getAllTemplates = findByRoute(routes, "GET /gitignore/templates");
    if (getAllTemplates) {
      getAllTemplates.operation["x-changes"].push({
        type: "operation",
        date: "2020-05-04",
        note:
          '"gitignore/list-templates" operation ID is now "gitignore/get-all-templates"',
        before: {
    ...
    const getAllCommonlyUsed = findByRoute(routes, "GET /licenses");
    if (getAllCommonlyUsed) {
      getAllCommonlyUsed.operation["x-changes"].push({
        type: "operation",
        date: "2020-05-04",
        note:
          '"licenses/list-commonly-used" operation ID is now "licenses/get-all-commonly-used"',
        before: {
          operationId: "licenses/list-commonly-used",
        },
        after: {
