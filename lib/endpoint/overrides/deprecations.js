...
function deprecations (state, endpoints) {
  ...
  const getCommit = findByRoute(endpoints, 'GET /repos/:owner/:repo/commits/:ref')
  ...
  //   endpoints.push(deprecatedGetCommitShaForRef)
  // }
  ...
  if (getCommit) {
    const newParam = getCommit.params.find(param => param.name === 'ref')
    const deprecatedParam1 = Object.assign({}, newParam, {
      name: 'sha',
      required: false,
      deprecated: {
        date: '2019-04-10',
    ...
    const deprecatedParam2 = Object.assign({}, newParam, {
      name: 'commit_sha',
      required: false,
      deprecated: {
        date: '2019-06-21',
    ...
    getCommit.params.push(deprecatedParam1, deprecatedParam2)
  }
  ...
  const listCommentsForCommit = findByRoute(endpoints, 'GET /repos/:owner/:repo/commits/:commit_sha/comments')
  if (listCommentsForCommit) {
    const newParam = listCommentsForCommit.params.find(param => param.name === 'commit_sha')
    const deprecatedParam = Object.assign({}, newParam, {
      name: 'ref',
      required: false,
      deprecated: {
    ...
    listCommentsForCommit.params.push(deprecatedParam)
  }

  // 2019-06-07 URL parameter "sha"  renamed to "commit_sha" for "Create a commit comment"
  const createCommitComment = findByRoute(endpoints, 'POST /repos/:owner/:repo/commits/:commit_sha/comments')
  if (createCommitComment) {
    const newParam = createCommitComment.params.find(param => param.name === 'commit_sha')
    const deprecatedParam = Object.assign({}, newParam, {
      name: 'sha',
      required: false,
      deprecated: {
    ...
    createCommitComment.params.push(deprecatedParam)
  }

  // 2019-06-07 "Create a file" & "Update a file" is now "Create or update a file"
  const createOrUpdateFile = findByRoute(endpoints, 'PUT /repos/:owner/:repo/contents/:path')
  if (createOrUpdateFile && createOrUpdateFile.idName === 'create-or-update-file') {
    const deprecatedCreateFile = Object.assign({}, createOrUpdateFile)
    const deprecatedUpdateFile = Object.assign({}, createOrUpdateFile)

    deprecatedCreateFile.name = 'Create a file'
    deprecatedCreateFile.idName = 'create-file'
    deprecatedCreateFile.deprecated = {
      date: '2019-06-07',
    ...
    endpoints.push(deprecatedCreateFile)

    deprecatedUpdateFile.name = 'Update a file'
    deprecatedUpdateFile.idName = 'update-file'
    deprecatedUpdateFile.deprecated = {
      date: '2019-06-07',
    ...
    endpoints.push(deprecatedUpdateFile)
  }
}
