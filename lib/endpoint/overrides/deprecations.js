...
function deprecations({ routes, gheVersion }) {
  ...
  if (getFeeds) {
    getFeeds.operation["x-changes"].push({
      ...
      date: "2020-03-25",
      note:
        '"activity/list-feeds" operation ID is now "activity/get-feeds"',
      before: {
  ...
  const markNotificationsAsRead = findByRoute(
    routes,
    "PUT /notifications"
  );
  ...
  const listPublicOrgEvents = findByRoute(
    routes,
    "GET /orgs/:org/events"
  );
  ...
  if (listEventsForAuthenticatedUser) {
    listEventsForAuthenticatedUser.operation["x-changes"].push({
      ...
      date: "2020-03-25",
      note:
        '"operations/activity/list-events-for-user" operation ID is now "activity/list-events-for-authenticated-user"',
      before: {
        operationId: "operations/activity/list-events-for-user",
      },
      after: {
