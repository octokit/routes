...
function deprecations({ routes, gheVersion }) {
  ...
  const checkAccountSubscribesToPlan = findByRoute(
    routes,
    "GET /marketplace_listing/accounts/:account_id"
  );
  if (checkAccountSubscribesToPlan) {
    checkAccountSubscribesToPlan.operation["x-changes"].push({
      type: "operation",
      date: "2020-03-04",
      note:
        '"apps/check-account-is-associated-with-any" operation ID is now "operations/apps/check-account-subscribes-to-plan"',
      before: {
        operationId: "apps/check-account-is-associated-with-any"
      },
      after: {
        operationId: "operations/apps/check-account-subscribes-to-plan"
      }
    });
  }
  ...
  if (getSubscriptionPlanForAccount) {
    getSubscriptionPlanForAccount.operation["x-changes"].push({
      ...
      date: "2020-03-08",
      note:
        '"apps/check-account-subscribes-to-plan" operation ID is now "apps/get-subscription-plan-for-account"',
      before: {
        operationId: "apps/check-account-subscribes-to-plan"
      },
      after: {
  ...
  if (getSubscriptionPlanForAccountStubbed) {
    getSubscriptionPlanForAccountStubbed.operation["x-changes"].push({
      ...
      date: "2020-03-08",
      note:
        '"apps/check-account-subscribes-to-plan-stubbed" operation ID is now "apps/get-subscription-plan-for-account-stubbed"',
      before: {
        operationId: "apps/check-account-subscribes-to-plan-stubbed"
      },
      after: {
