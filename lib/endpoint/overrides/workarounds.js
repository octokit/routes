...
function workarounds(state) {
  state.routes.forEach((route) => {
    ...
    // `"repository_selection"` key is a string that can have only one of two values
    // https://github.com/octokit/routes/issues/755
    if (route.operation.responses) {
      Object.values(route.operation.responses).forEach((response) => {
        if (!response.content) {
          return;
        }
        Object.values(response.content).forEach((content) => {
          valuesDeep(content.schema, (value) => {
            if (!isObject(value)) return;

            if ("repository_selection" in value) {
              value.repository_selection.enum = ["all", "selected"];
            }
          });
        });
      });
...
function valuesDeep(obj, iterator) {
  if (isObject(obj)) {
    return Object.values(obj).forEach((value) => {
      iterator(value);
      valuesDeep(value, iterator);
    });
  }
  if (isArray(obj)) {
