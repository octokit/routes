...
function workarounds (state) {
  state.results.forEach(result => {
    ...
    if (/^\/repos\/:owner\/:repo\/pre-receive-hooks/.test(result.path)) {
      result.name += ' for repository'
    }
    if (/^\/orgs\/:org\/pre-receive-hooks/.test(result.path)) {
      result.name += ' for organization'
    }

    // https://github.com/octokit/routes/issues/332
    if (state.gheVersion <= 2.15 && route === 'POST /repos/:owner/:repo/issues/:number/labels') {
      result.params.push({
        'mapTo': 'input',
        'name': 'labels',
        'type': 'string',
