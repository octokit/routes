module.exports = findRoutesWithOverrides

const _ = require('lodash')

const routesOverrides = require('./overrides/routes')

function findRoutesWithOverrides (state) {
  findRoutes(state)

  // avoid duplicate names
  // https://github.com/octokit/routes/issues/48
  state.results.forEach(result => {
    const route = [result.method, result.path].join(' ')
    if (routesOverrides[route]) {
      const [newMethod, newPath] = routesOverrides[route].split(' ')
      result.method = newMethod
      result.path = newPath
    }
  })
}
...
function findRoutes (state) {
  ...
  const hasExampleRoutes = !!routeBlocks.slice(1).find(block => {
    ...
    if (/^You can filter/.test(prevBlock.text)) {
      return true
    }
  ...
  if (isRepositoryCreate) {
    ...
    const swap = state.blocks.slice(0, 2)
    state.blocks[0] = swap[1]
    state.blocks[1] = swap[0]
  }
