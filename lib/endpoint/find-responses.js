...
function findRespones (state) {
  const responseTitleBlocks = state.blocks.filter(block => block.type === 'responseTitle')
...
function findResponse (state, responses) {
  ...
  for (var i = 0; i < blocksAfterResponseTitle.length; i++) {
    ...
    let description = responseTitleBlock.text === 'Response' ? null : responseTitleBlock.text
    let body = null
