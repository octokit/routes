...
module.exports = {
  ...
  parse(el) {
    ...
    const isRequiredForAccess = /to access (the([^,.]*)|this)( new)? (API|endpoint)/i.test(
      text
    );

    if (matches) {
      // workarounds, can be removed once the wording in the docs is clarified
      const isAntManException =
        matches[1] === "ant-man" &&
        /^This endpoint is part of the deployment and deployment status enhancement/.test(
          text
        );
      const isHellcatException =
        matches[1] === "hellcat" &&
        /At this time, the hellcat-preview media type is required to use this endpoint/.test(
          $el.parent().text()
        );
      const isNotRequired =
        !isAntManException &&
        !isHellcatException &&
        /^(hellcat|ant-man)$/.test(matches[1]);
