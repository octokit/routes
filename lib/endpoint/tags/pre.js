...
module.exports = {
  ...
  parse (el) {
    ...
    if ($el.is('.highlight-json')) {
      ...
      if ($el.prev().is('.highlight-headers') || $el.prevAll('h3').first().text().trim() === 'Response') {
        return {
          type: 'response',
          format: 'json',
          data
      ...
      return {
        type: 'inputExample',
        format: 'json',
        title: $el.prevUntil('h3', 'h4').first().text().trim() || $el.prevAll('h3').first().text().trim(),
        data
      }
    }
