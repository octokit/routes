...
function isParametersTable($table) {
  const hasParametersHeaders =
    $table.find("thead").text().replace(/\s/g, "") === "NameTypeDescription";
...
function toParams($table) {
  const params = $table.find("tbody tr").map(rowToParameter).get();
  ...
  if (
    params.filter((param) => /\./.test(param.name)).length === params.length
  ) {
    params.unshift({
...
function rowToParameter(i, el) {
  const [
    ...
    .map((i, el) => {
      ...
      // Sometimes preview headers are mentioned in parameter descriptions. They might end up in
      // the list of enum values
      enumValues = enumValues.filter((value) => !/-preview$/.test(value));
  ...
  if (/ or null/.test(param.type)) {
    param.allowNull = true;
    param.type = param.type.replace(/ or null/, "");
  }
