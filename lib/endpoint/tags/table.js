...
function rowToParameter (i, el) {
    ...
    .map((i, el) => {
      ...
      if (hasDefault) {
        ...
        if (/^[a-z]/.test(afterDefaultDescription)) {
          const [defaultDescription, ...rest] = afterDefaultDescription.trim().split(/\.[\s\n]/)
      ...
      let enumValues = []
      if (CAN_BE_ONE_OF_REGEXP.test(description) && !NOT_COMMA_SEPARATED_REGEXP.test(description)) {
  ...
  if (param.name === 'type') {
    if (/Normally this is a `commit` but it can also be a `tree` or a `blob/.test(param.description) === true) {
      param.enum = [
        'commit',
        'tree',
        'blob']
  ...
  if (param.name === 'default_repository_permission') {
    if (/Default permission level members have for organization repositories:/.test(param.description) === true) {
      param.enum = [
        'read',
        'write',
        'admin',
  ...
  if (/\[\w+\]/.test(param.name)) {
    const [, parentParamName, childParamName] = param.name.match(/^(\w+)\[(\w+)/)
