...
function findExamples(state) {
  const exampleBlocks = state.blocks.filter(
    block => block.type === "inputExample"
  );
  const responseBlocks = state.blocks.filter(
  ...
  const hasMultipleExamples =
    exampleBlocks.length > 1 && state.routes.length !== exampleBlocks.length;
  const responses = responseBlocks.map(buildResponse, { state });
  const examples = exampleBlocks.map(buildExample, {
    state,
    hasMultipleExamples,
    responses
  });
  state.routes.forEach((route, routeIndex) => {
    ...
    for (let i = 0, n = routeExamples.length - 1; i <= n; i++) {
      ...
      for (const paramName of Object.keys(example.request.params)) {
        ...
        let param = route.operation.parameters.find(
    ...
    responses.forEach(response => {
      ...
      if (count > 1 || (responses.length > 1 && !title)) {
        if (!title) {
          responseTitle = count > 1 ? `Response ${index}` : "Default response";
...
function buildResponse(responseBlock, i, responseBlocks) {
  ...
  for (const block of potentialResponseBlocks) {
    if (["responseTitle", "alternativeResponseTitle"].includes(block.type)) {
      blocks.title = block;
      break;
...
function formatResponse(state, blocks) {
  const { header, preview, mediaType, title, descriptions } = blocks;
  ...
  if (
    title &&
    title.text &&
    !["response", "successful response"].includes(
      title.text.toLowerCase().trim()
    )
  ) {
...
function buildExample(inputBlock, i, exampleBlocks) {
  ...
  const potentialResponseBlocks = state.blocks.slice(
    inputBlockIndex + 1,
    inputBlockIndex + 6
  );
  ...
  for (const block of potentialResponseBlocks) {
    if (
      ["responseTitle", "alternativeResponseTitle", "responseHeaders"].includes(
        block.type
      )
    ) {
      // do nothing
      ...
      ["exampleTitle", "alternativeParametersTitle", "inputExample"].includes(
        block.type
      )
    ) {
...
function addBodyParamExample(example, jsonContent, paramName, paramValue) {
  if (example.key) {
    ...
    jsonContent.examples[key] =
      jsonContent.examples[key] || getExampleObject({}, title);
...
function addResponseExample(key, title, response, responses) {
  ...
  if (key || responseContent.examples) {
    ...
    const responseTitle =
      ...
      response.title &&
      title !== "response" &&
      title.toLowerCase() !== response.title.toLowerCase()
