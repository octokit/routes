...
function findInBlocks (state) {
  ...
  const params = parametersBlocks[0].params.map(formatParam, route)
  ...
  parametersBlocks.slice(1).forEach(parametersBlock => {
    ...
    if (alternativeHeaderBlock) {
      ...
      const params2 = parametersBlock.params.map(formatParam, route2)
      route.operation.parameters.push(...params)
    ...
    parameterNames.forEach(parameterName => {
      ...
      const schema = parentParam.type === 'array'
        ? parentParam.items
        : parentParam
      parametersBlock.params.map(formatParam, route).map(param => {
...
function formatParam (param) {
  const { method, path, operation } = this
...
function buildObjectSchema (param, params) {
  ...
  schema.properties = {} // childParams.map(buildSchema)
