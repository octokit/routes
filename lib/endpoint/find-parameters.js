...
function makeRelativeLinksAbsolute (state) {
  state.results.forEach(output => {
    for (let i = 0; i < output.params.length; i++) {
      const param = output.params[i]
      param.description = normalizeMarkdown(state, param.description)
      if (param.default && param.type === 'string') {
        param.default = normalizeMarkdown(state, param.default)
      }
    }
    return output
  })
...
function mapToInput (state) {
  state.results.forEach(endpoint => {
    ...
    const mapToInputParam = PARAMETERS_TO_INPUT[route]
