...
function mapToInput (state) {
  state.results.forEach(endpoint => {
    ...
    const mapToInputParam = state.gheVersion && PARAMETERS_TO_INPUT[state.gheVersion][route]
      ? PARAMETERS_TO_INPUT[state.gheVersion][route]
      : PARAMETERS_TO_INPUT.common[route]
