...
function convertEndpointToOperation (params) {
  ...
  // e.g. oauth-authorizations/get-grant
  const operationId = [paramCase(scope), paramCase(endpoint.idName)].join('/')
...
function toJsExample ({ operationId, endpoint }) {
  ...
  // e.g. oauth-authorizations/get-grant -> oauthAuthorizations.getGrant
  const operationPath = operationId.split('/').map(piece => camelCase(piece)).join('.')

  return `octokit.${operationPath}(${Object.keys(params).length ? stringify(params, null, 2) : ''})`
}
