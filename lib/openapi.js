...
function getIdName(endpoint, scope) {
  ...
  switch (route) {
    ...
    case "DELETE /app/installations/:installation_id":
      return "delete-installation";
    case "GET /orgs/:org/installation":
      return "get-org-installation";
    case "GET /repos/:owner/:repo/installation":
      return "get-repo-installation";
    case "GET /users/:username/installation":
      return "get-user-installation";
    case "GET /app/installations":
      return "list-installations";
    case "GET /meta":
      return "get";
    case "GET /repos/:owner/:repo/comments":
      return "list-commit-comments-for-repo";
    case "GET /orgs/:org/blocks":
      return "list-blocked-users";
    case "GET /user/following":
      return "list-followed-by-authenticated";
    case "GET /repos/:owner/:repo/pulls/:pull_number/reviews/:review_id/comments":
      return "list-comments-for-review";
    // https://github.com/octokit/routes/pull/769#pullrequestreview-425786418
    case "GET /repos/:owner/:repo/commits/:commit_sha/comments":
      return "list-comments-for-commit";
  }
  ...
  // git-ref -> ref
  idName = idName.replace(/git-ref/, "ref");

  // misc
  idName = idName.replace(/contextual-information/, "context");
  idName = idName.replace(/-from-legacy$/, "-legacy");
  ...
  // remove code-scanning from ID when scope is code-scanning
  ...
  if (idName === "create-review-comment-for-alternative") {
    return "create-review-comment-alternative";
  }
...
function assignResponse(
  ...
  if (headers && headers.length) {
    ...
    for (const { name, example } of headers) {
      response.headers[name] = { example, schema: getHeaderSchema(name) };
    }
  }

  function getHeaderSchema(name) {
    switch (name) {
      case "X-RateLimit-Limit":
      case "X-RateLimit-Remaining":
        return { type: "integer" };
      case "X-RateLimit-Reset":
        return { type: "integer", format: "timestamp" };
      default:
        return { type: "string" };
    }
  }
