...
function getIdName (route, scope) {
  ...
  switch (endpoint) {
    ...
    case 'GET /repos/:owner/:repo/pulls/:pull_number/merge': return 'check-if-merged'
    ...
    case 'PATCH /teams/:team_id/discussions/:discussion_number/comments/:comment_number': return 'update-discussion-comment'
    ...
    case 'GET /repos/:owner/:repo/issues/:issue_number/timeline': return 'list-events-for-timeline'
    case 'GET /user/blocks/:username': return 'check-blocked'
    case 'DELETE /orgs/:org/migrations/:migration_id/repos/:repo_name/lock': return 'unlock-repo-for-org'

    // https://github.com/octokit/routes/issues/174
    case 'GET /user/installations': return 'list-installations-for-authenticated-user'
    case 'GET /user/installations/:installation_id/repositories': return 'list-installation-repos-for-authenticated-user'
    case 'GET /user/marketplace_purchases': return 'list-marketplace-purchases-for-authenticated-user'
    case 'GET /user/marketplace_purchases/stubbed': return 'list-marketplace-purchases-for-authenticated-user-stubbed'
    case 'POST /user/migrations': return 'start-for-authenticated-user'
    ...
    case 'DELETE /user/migrations/:migration_id/archive': return 'delete-archive-for-authenticated-user'
    case 'DELETE /user/migrations/:migration_id/repos/:repo_name/lock': return 'unlock-repo-for-authenticated-user'
    case 'GET /user/teams': return 'list-for-authenticated-user'
    case 'GET /user/repository_invitations': return 'list-invitations-for-authenticated-user'
    case 'GET /user/following': return 'list-following-for-authenticated-user'
    case 'GET /user/issues': return 'list-for-authenticated-user'
    case 'GET /user/subscriptions': return 'list-watched-repos-for-authenticated-user'
    case 'GET /user/followers': return 'list-followers-for-authenticated-user'
    case 'GET /user/orgs': return 'list-for-authenticated-user'
    case 'GET /user/memberships/orgs/:org': return 'get-membership-for-authenticated-user'
    ...
    case 'GET /repos/:owner/:repo/milestones/:milestone_number/labels': return 'list-labels-for-milestone'
    ...
    case 'PUT /repos/:owner/:repo/pulls/:pull_number/merge': return 'merge'
    ...
    case 'GET /scim/v2/organizations/:organization_id/Users': return 'get-provisioned-identities-list'
    ...
    case 'GET /repos/:owner/:repo/issues/:issue_number/labels': return 'list-labels-on-issue'
    case 'GET /apps/:app_slug': return 'get-by-slug'
    case 'POST /user/projects': return 'create-for-authenticated-user'
    case 'GET /repos/:owner/:repo/vulnerability-alerts': return 'check-vulnerability-alerts'
    case 'POST /orgs/:org/repos': return 'create-in-org'
    case 'POST /user/repos': return 'create-for-authenticated-user'
  }

  // workaround for stats endpoints: deviate idName for path
  if (/\/stats\//.test(endpoint)) {
    const statIdName = kebabCase(endpoint.split(/\/stats\//)[1])
  ...
  const ignoreWordsAtEndRegex = new RegExp(`\\b(${IGNORE_WORDS_AT_END.join('|')})\\s*$`, 'i')

  let idName = route.operation.summary
...
function assignResponse (responses, code, description, body, mediaType) {
  ...
  if (!response.description) {
    response.description = description || (intCode === 404 ? 'Not Found' : 'response')
  }
  if (body) {
