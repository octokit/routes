...
async function checkOrUpdateRoutes (options) {
  ...
  await Object.keys(routesByScope).reduce(async (promise, scope) => {
    ...
    const path = joinPath(__dirname, '..', 'routes', 'api.github.com', filename)
    await writeFile(path, JSON.stringify(routesByScope[scope], null, 2) + '\n')
    console.log(`✅  routes/api.github.com/${filename} written`)
  }, null)

  await writeFile('routes/api.github.com/index.json', JSON.stringify(routesByScope, null, 2) + '\n')
  console.log(`✅  routes/api.github.com/index.json written`)
