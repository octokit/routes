...
async function checkOrUpdateRoutes (options) {
  ...
  await urls.reduce(async (promise, url) => {
    ...
    console.log(`🌐  ${url}`)
    // TODO: cache page HTML instead of getting loading from URL/reading it from file each time
    ...
    await endpoints.reduce(async (promise, endpoint) => {
      ...
      const endpointDiffs = diff(endpoint, existingEndpoint)
      if (!endpointDiffs) {
      ...
      outOfDate.push({
        ...
        diff: existingEndpoint && diffString(endpoint, existingEndpoint),
